            ...
            if cmd_lower == keyword_name.lower() or (is_alias and cmd_lower == target_name):
                ...
                if original_keyword_name == "교수님":
                    ...
                    if self.inventory.has("법인카드"):
                        ...
                        self.ui.print_system_message("이 문으로 나갈 수 있을 것 같다. 하지만 아직 무언가 잊은 기분이 든다.")
                    ...
                    self.ui.print_narrative("교수님의 책상이다. 각종 서류와 논문이 어지럽게 널려있다.", is_markdown=True)
                elif original_keyword_name == "안경알":
                    self.ui.print_narrative("교수님의 안경알이 빛을 번뜩인다. 저 너머의 눈은 웃고 있는지, 화를 내고 있는지 알 수 없다.", is_markdown=True)
                else:
